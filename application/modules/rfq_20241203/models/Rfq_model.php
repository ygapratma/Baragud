...
class Rfq_model extends CI_Model
{
     ...
     * @return void
     */
    public function insertEquivalent_old($data = array())
    {
	...
	public function insertEquivalent($data = array())
    {
        ...
        if ($nextEquivalent == (int)$data['ekuivalen']) {
            // Gunakan SQL Binding untuk menghindari error karakter
            $sql = "INSERT INTO " . $this->table[2] . " 
                (nomor_rfq, urutan_rfq, ekuivalen, kode_barang, deskripsi_barang, deskripsi_material, deskripsi, jumlah_permintaan, satuan, deskripsi_satuan, nomor_sr, kode_kebun, mata_uang, harga_satuan, per_harga_satuan, konversi, jumlah_konversi, satuan_konversi, ketersediaan_barang, masa_berlaku_harga, keterangan, dibuat_oleh, modified_date, modified_by, spesifikasi, merek, tipe, buatan, jumlah_tersedia, jumlah_inden, lama_inden) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            $binds = array(
                (string) $data['nomor_rfq'], 
                (is_null($data['urutan_rfq']) ? NULL : (int) $data['urutan_rfq']), 
                (int) $data['ekuivalen'], 
                (string) $data['kode_barang'], 
                (string) $data['deskripsi_barang'], 
                (string) $data['deskripsi_material'], 
                (string) $data['deskripsi'], 
                (float) $data['jumlah_permintaan'], 
                (string) $data['satuan'], 
                (string) $data['deskripsi_satuan'], 
                (string) $data['nomor_sr'], 
                (string) $data['kode_kebun'], 
                (string) $data['mata_uang'], 
                (float) $data['harga_satuan'], 
                (string) $data['per_harga_satuan'], 
                (string) $data['konversi'], 
                (is_null($data['jumlah_konversi']) ? NULL : (float) $data['jumlah_konversi']), 
                (string) $data['satuan_konversi'], 
                (string) $data['ketersediaan_barang'], 
                (string) $data['masa_berlaku_harga'], 
                (string) $data['keterangan'], 
                (string) $data['dibuat_oleh'], 
                (string) $data['modified_date'], 
                (string) $data['modified_by'], 
                (string) $data['spesifikasi'], 
                (string) $data['merek'], 
                (string) $data['tipe'], 
                (string) $data['buatan'], 
                (float) $data['jumlah_tersedia'], 
                (float) $data['jumlah_inden'], 
                (int) $data['lama_inden']
            );
