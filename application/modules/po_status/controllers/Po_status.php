...
class Po_Status extends CI_Controller {
    ...
    public function create_template($po_number = '')
    {
        ...
        $sheet->setCellValue('I3', "Expiry Date (MM/dd/yyyy)");
        $sheet->setCellValue('J3', "Manufacture Date (MM/dd/yyyy)");
        ...
        foreach($details as $data){ // Lakukan looping pada variabel siswa
            ...
            $sheet->setCellValue('E'.$numrow, utf8_encode($data->short_text));
            ...
            for ($row = 4; $row <= $highestRow; ++$row) {
                ...
                if($rowData[$row]['B'] == $nomor_po) {
                    ...
                    $paramItem  = [
                        ...
                        'short_text' => utf8_decode($rowData[$row]['E']),
                        'unit' => $rowData[$row]['G'],
                    ];
                    ...
                    if($getPOItem->num_rows() > 0) {
                        ...
                        $rows   = [
                            ...
                            'batch' => $rowData[$row]['H'],
                            'kadarluarsa' => (!empty($rowData[$row]['I'])) ? date('m/d/Y', strtotime($rowData[$row]['I'])) : '',
                            'tanggal_produksi' => (!empty($rowData[$row]['J'])) ? date('m/d/Y', strtotime($rowData[$row]['J'])) : '',
        ...
        foreach($upload_data as $key => $value) {
            foreach($value as $valKey => $val) {
                if($valKey == 'id') {
                    ...
                    if($valKey == 'kadarluarsa' || $valKey == 'tanggal_produksi') {
                        $xp = (!empty($val)) ? explode("/", $val) : 'NULL';
                        $upload_data[$key][$valKey] = (!empty($val)) ? ($xp[2]."-".$xp[0]."-".$xp[1]) : 'NULL';
                    } else if($valKey == 'deskripsi_material') {
                        $upload_data[$key][$valKey] = utf8_decode($val);
                    }
        ...
        if($getPOBatchData->num_rows() > 0) {
            ...
            foreach($upload_data as $key => $value) {
                if($value['batch'] == $po_batch[$key]->batch) {
                    $xp1    = (!empty($value['kadarluarsa'])) ? explode("/", $value['kadarluarsa']) : 'NULL';
                    $xp2    = (!empty($value['tanggal_produksi'])) ? explode("/", $value['tanggal_produksi']) : 'NULL';;
                    $data_update = [
                        // 'kadarluarsa' => (!empty($value['kadarluarsa'])) ? $xp1[2]."-".$xp1[0]."-".$xp1[1] : 'NULL',
                        // 'tanggal_produksi' => (!empty($value['tanggal_produksi'])) ? $xp2[2]."-".$xp2[0]."-".$xp2[1] : 'NULL'
                        'kadarluarsa' => (!empty($value['kadarluarsa'])) ? date('m/d/Y', strtotime($value['kadarluarsa'])) : '',
                        'tanggal_produksi' => (!empty($value['tanggal_produksi'])) ? date('m/d/Y', strtotime($value['tanggal_produksi'])) : ''
                    ];
